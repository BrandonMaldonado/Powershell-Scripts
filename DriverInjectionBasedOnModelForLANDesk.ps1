...
try {
    $computerModel = (Get-CimInstance -ClassName Win32_ComputerSystem | Select-Object Model).('Model')
}
catch {
    Write-Host -ForegroundColor Red "Error: Unable to get device model. Please attempt to grab the device model manually."
    Pause
    exit
}
...
if ($mappedDriveFound) {
    ...
    try {
        ...
        Write-Host -ForegroundColor red "Error: Unable to get the DriverMatrix CSV from $UNCPath"
        Pause
        exit
    }
    ...
    $driverTable.GetEnumerator() | ForEach-Object {
        ...
        if ($computerModel -like $modelFamily) {
            ...
            try {
                ...
                Write-Host -ForegroundColor Red "Error: Did not find a driver pack $($SPvalue+".zip") for model $computerModel on $UNCPath (check the provisioning\drivers folder to see if it exists)"
                Pause
                exit
            }
